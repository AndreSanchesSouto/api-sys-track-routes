...
public class DeliveryProductService {
    ...
    public DeliveryProductEntity getByDeliveryIdAndProductId(UUID deliveryId, UUID productId) {
        return this.repository.findByDeliveryIdAndProductId(deliveryId, productId).orElseThrow(
                () -> new DeliveryException("Não foi localizado esse produto na entrega")
        );
    }
    ...
    @Transactional
    public void deleteByDeliveryIdAndProductId(UUID deliveryId, UUID productId) {
        this.repository.deleteByDeliveryIdAndProductId(deliveryId, productId);
    }
}
